...
public class LocationLast {
	...
	public void getLastLocation() {
		...
		for (int i = 0; i < mProviderList.size(); i++) {
			mTempLocation = mLocationManager.getLastKnownLocation(mProviderList
					.get(i));
			if (mTempLocation != null) {
				...
				int accuracy = (int) Math.ceil(mLocation.getAccuracy()
						- mTempLocation.getAccuracy());
	...
	// ///// ******** Asynctask to get info using geoocoder by passing latitude
			...
			if (list != null)
				try {
					if (list.get(0).getFeatureName() != null
						...
						MainActivity.mCurrentLocation = list.get(0)
								.getFeatureName()
								+ ", "
